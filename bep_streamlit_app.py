...
# 各予測タブごとの初期値プリセット（予測1, 2, 3）
presets = {
    1: dict(sales=500, rent=100, salary=100, key_money=100, deposit=100,
            guarantee=100, agency=100, interior=100, others=100,
            months=12, margin=0.64, utilities=7, tax_rate_percent=10),
    2: dict(sales=500, rent=110, salary=110, key_money=100, deposit=100,
            guarantee=100, agency=100, interior=100, others=100,
            months=12, margin=0.64, utilities=7, tax_rate_percent=10),
    3: dict(sales=500, rent=90, salary=90, key_money=100, deposit=100,
            guarantee=100, agency=100, interior=100, others=0,
            months=12, margin=0.64, utilities=7, tax_rate_percent=10),
}

for idx, tab in enumerate(tabs, start=1):
    if idx <= 3:
        preset = presets[idx]  # 👈 各予測ごとの初期値セット
        ...
        with tab:
            ...
            with left_col:
                ...
                sales = st.number_input("月間売上（税込）[万円]", value=preset["sales"], step=10, key=f"sales_{idx}")

                st.markdown(f"### 🗓️ 月間固定費", unsafe_allow_html=True)
                rent = st.number_input("家賃（月）[万円][課税]", value=preset["rent"], step=10, key=f"rent_{idx}")
                salary = st.number_input("人件費（月）[万円]", value=preset["salary"], step=10, key=f"salary_{idx}")
                ...
                key_money = st.number_input("礼金 [万円][課税]", value=preset["key_money"], step=10, key=f"key_money_{idx}")
                deposit = st.number_input("敷金 [万円]", value=preset["deposit"], step=10, key=f"deposit_{idx}")
                guarantee_money = st.number_input("保証金 [万円]", value=preset["guarantee"], step=10, key=f"guarantee_{idx}")
                agency_fee = st.number_input("仲介手数料 [万円][課税]", value=preset["agency"], step=10, key=f"agency_{idx}")
                interior_cost = st.number_input("内装工事費 [万円][課税]", value=preset["interior"], step=10, key=f"interior_{idx}")
                others = st.number_input("その他費用 [万円][課税]", value=preset["others"], step=10, key=f"others_{idx}")

                initial_cost_display = (key_money * tax_rate + deposit + guarantee_money +
                                        agency_fee * tax_rate + interior_cost * tax_rate + others * tax_rate)
                ...
                with st.expander("詳細設定", expanded=False):
                    months = st.slider("シミュレーション月数", 1, 24, value=preset["months"], step=1, key=f"months_{idx}")
                    contribution_margin = st.slider("粗利率", min_value=0.4, max_value=0.8, value=preset["margin"], step=0.01, key=f"margin_{idx}")
                    utilities = st.number_input("光熱費・水道代・通信費（月）[万円]", value=preset["utilities"], step=1, key=f"utilities_{idx}")
                    tax_rate_percent = st.number_input("消費税率 [%]", value=preset["tax_rate_percent"], step=1, key=f"tax_rate_percent_{idx}")
